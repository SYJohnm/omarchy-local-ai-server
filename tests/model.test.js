...
const PiConfig = require("../PiConfig.js")

// A hand-written catalogue, including a compat block the plugin cannot derive
// and a second provider it does not own.
const PI_EXISTING = JSON.stringify({
  providers: {
    "local-llama": {
      name: "My llama",
      ...
      models: [{ id: "old", name: "stale", contextWindow: 16384 }]
    },
    anthropic: { name: "Anthropic", models: [{ id: "claude" }] }
  }
})

test("context window follows the launch setting, not the model's maximum", () => {
  // The drift that makes a hand-written catalogue wrong: the server enforces
  // what it was started with, regardless of what the model could support.
  assert.strictEqual(PiConfig.effectiveContext("65536", 262144), 65536)
  // 0 means "use the model's own", for both backends.
  assert.strictEqual(PiConfig.effectiveContext("0", 262144), 262144)
  assert.strictEqual(PiConfig.effectiveContext("", 0), 4096, "a usable floor when nothing is known")
})

test("capabilities come from the probed model", () => {
  const m = PiConfig.buildModel({
    id: "/models/x.gguf", name: "x",
    caps: { thinking: true, vision: true },
    launchCtxSize: "32768", trainedContext: 262144
  })
  assert.strictEqual(m.reasoning, true)
  assert.deepStrictEqual(m.input, ["text", "image"])
  assert.strictEqual(m.contextWindow, 32768)
  assert.ok(m.maxTokens < m.contextWindow, "output cap must leave room for the prompt")

  const plain = PiConfig.buildModel({ id: "y", caps: {}, launchCtxSize: "4096" })
  assert.strictEqual(plain.reasoning, false)
  assert.deepStrictEqual(plain.input, ["text"])
})

test("merging preserves other providers and hand-written compat", () => {
  const cat = PiConfig.parseCatalogue(PI_EXISTING)
  const provider = PiConfig.buildProvider(
    { backend: "llamacpp", host: "127.0.0.1", port: "8080", models: [PiConfig.buildModel({ id: "new", launchCtxSize: "65536" })] },
    PiConfig.existingProvider(cat, "llamacpp"))
  const merged = PiConfig.mergeCatalogue(cat, "llamacpp", provider)

  assert.ok(merged.providers.anthropic, "an unrelated provider survives")
  assert.strictEqual(merged.providers.anthropic.models[0].id, "claude")
  assert.deepStrictEqual(merged.providers["local-llama"].compat,
    { thinkingFormat: "qwen-chat-template", supportsDeveloperRole: false },
    "compat cannot be derived, so it must be carried through")
  assert.strictEqual(merged.providers["local-llama"].name, "My llama", "a custom name is kept")
  assert.strictEqual(merged.providers["local-llama"].models.length, 1)
  assert.strictEqual(merged.providers["local-llama"].models[0].id, "new", "stale entry replaced")
})

test("each backend writes its own provider key", () => {
  assert.strictEqual(PiConfig.providerKey("llamacpp"), "local-llama")
  assert.strictEqual(PiConfig.providerKey("ollama"), "local-ollama")

  const cat = PiConfig.parseCatalogue(PI_EXISTING)
  const merged = PiConfig.mergeCatalogue(cat, "ollama",
    PiConfig.buildProvider({ backend: "ollama", host: "h", port: "11434", models: [] }, null))
  assert.ok(merged.providers["local-llama"], "the llama provider is untouched by an ollama sync")
  assert.strictEqual(merged.providers["local-ollama"].baseUrl, "http://h:11434/v1")
})

test("a corrupt catalogue is refused rather than overwritten", () => {
  // Overwriting would discard every provider the user configured.
  assert.strictEqual(PiConfig.parseCatalogue("{not json"), null)
  assert.strictEqual(PiConfig.parseCatalogue("[1,2,3]").length, 3, "valid JSON is accepted as-is")
  assert.deepStrictEqual(PiConfig.parseCatalogue(""), { providers: {} }, "absent means start fresh")
})
